...
const CHOICES: { id: Choice; emoji: string; label: string }[] = [
  ...
  { id: "scissors", emoji: "✂", label: "Scissors" },
];
...
export default function RpsScreen() {
  ...
  // ADDED: Local lifecycle state handlers tracking matches for the interstitial trigger
  const [gamesPlayed, setGamesPlayed] = useState(0);
  const [adLoaded, setAdLoaded] = useState(false);

  // CHANGED: Combined storage initialization hooks with our multiplatform ad listener stream
  useEffect(() => {
  ...
  const play = (choice: Choice) => {
    ...
    // ADDED: Track the matches and trigger the interstitial ad on every 3rd completed round
  ...
  const reset = () => {
    ...
    // Optional: setGamesPlayed(0); // Uncomment if you want to reset the ad counter when clicking reset
  };
      ...
      {(score.wins > 0 || score.losses > 0 || score.draws > 0) && (
        <View style={styles.shareRow}>
          <ShareButton
            ...
            line={`RPS vs CPU on Tiny Arcade — ${score.wins} W / ${score.draws} D / ${score.losses} L ✊✋✌`}
